...
ipcMain.on('inject-input', (_event, message) => {
  if (!message || !['mouse-move', 'mouse-relative', 'mouse-button', 'mouse-down', 'mouse-up', 'mouse-click', 'mouse-scroll', 'key-down', 'key-up', 'text', 'release-input', 'host-alt-tab', 'set-desklink-osk-mode', 'set-interception-keyboard-mode', 'gamepad-state'].includes(message.type)) return;
